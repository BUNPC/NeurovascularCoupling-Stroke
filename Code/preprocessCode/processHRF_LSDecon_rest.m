...
load([dataDir,'/','3_v2_rsfc_I.mat'])
HbT = permute(Iunfilt,[1 2 4 3]);
load([dataDir,'/','gcampcorr_v2_rsfc_I.mat'])
deltaGCaMPcorr = permute(Iunfilt,[1 2 4 3]);
% sub = dir(dataDir);
...
save([dataDir,'/','rest_HRF.mat'],'HRF','peakTime','amplitudePos','amplitudeNeg','fwhm','pHbT','correlation','SSE','RMSE')

end

%% Calculation based on HRF

% dataDir = uigetdir('Please select the Data folder');
% load([dataDir,'\','3_rsfc_I.mat'])
% HbT = permute(I,[1 2 4 3]);
% load([dataDir,'\','gcampcorr2_rsfc_I.mat'])
% deltaGCaMPcorr = permute(I,[1 2 4 3]);
% sub = dir(dataDir);
% dfolders = sub([sub(:).isdir]);
% dfolders = dfolders(~ismember({dfolders(:).name},{'.','..'}));
% load([dataDir,'\','rest_HRF2.mat'])
Fs = 5;
T = 1/Fs;
L = size(HbT,3);
timefull = (1:(L))*T;
...
for t = 1:3
    ...
    mcorr_roi = mean(mean(correlation(y(1):y(2),x(1):x(2))));
    ...
    plot_areaerrorbar(HRF_roi(:,4:end),options)
    xlabel('Time (sec)')
    title('IRF')
...
%     annotation('textbox',dim,'String',str,'FitBoxToText','on');
    ...
    subplot(3,3,t+6)
    plot(timefull, squeeze(mean(tHbT_roi)),'LineWidth',1)
    hold on
    plot(timefull(start:stop), squeeze(mean(pHbT_roi)),'LineWidth',1)
    ylim([-5*1e-6 8*1e-6])
    ylabel('Change in HbT (\muM)')
    yyaxis right
    plot(timefull, squeeze(mean(gcamp_roi)),'LineWidth',1)
    legend('Measured HbT','Predicted HbT','Measured GCaMP')
    ylim([-0.05 0.15])
    xlabel('Time (sec)')
...
saveas(gcf,[filepath1,'/','IRF2 ',name1,'.tif'])
...
tGCaMPun = squeeze(deltaGCaMP(y1,x1,start:stop));
...
h = (((X'*X) + (lambda.*I))^-1) * (X'*Y);
condnum_x = cond((X'*X));
condnum_full = cond(((X'*X) + (lambda.*I)));
HRF = h; 

ptHbT = conv(HRF(5:end), tGCaMP);
ptHbT = conv(HRF(4:end), tGCaMP);

% ptHbT = ptHbT(start:stop) +  app*HRF(1:4);
ptHbT = ptHbT(start:stop) +  app*HRF(1:3);
...
%     plot(time(1:frames),HRF(5:end))
    ...
    yyaxis right
    plot(timefull(1:length(ptHbT)), tGCaMP)
    hold on
    plot(timefull(1:length(ptHbT)), tGCaMPun)
